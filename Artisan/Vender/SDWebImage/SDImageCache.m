...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        diskCachePath = SDWIReturnRetained([[[paths objectAtIndex:0] stringByAppendingPathComponent:@"ArtisanCache"] stringByAppendingPathComponent:@"SessionStore"]);
...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
   // [_lock lock];
    ...
    image = SDScaledImageForPath(key, [NSData dataWithContentsOfFile:path]);
    if (!image) {
