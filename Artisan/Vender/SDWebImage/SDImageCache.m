...
static const NSInteger kSDWebImageCacheLimit = 1024 * 1024 * 10;
...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        _memCache.name = @"me.roger.ZenCache";
        _memCache.totalCostLimit = kSDWebImageCacheLimit;

        // Init the disk cache
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
        diskCachePath = SDWIReturnRetained([[[paths objectAtIndex:0] stringByAppendingPathComponent:@"ZenCache"] stringByAppendingPathComponent:@"SessionStore"]);
...
- (void)notifyDelegate:(NSDictionary *)arguments
{
    ...
    if (image)
    {
        [_memCache setObject:image forKey:key cost:image.size.height * image.size.width * image.scale];
...
- (void)queryDiskCacheOperation:(NSDictionary *)arguments
{
   // [_lock lock];
    ...
    UIImage *image = nil;
    NSString *path = [diskCachePath stringByAppendingPathComponent:key];
    ...
    if ([data isGif]) {
        ...
        image = SDScaledImageForPath(key, data);
...
- (void)storeImage:(UIImage *)image imageData:(NSData *)data forKey:(NSString *)key toDisk:(BOOL)toDisk
{
    ...
    if (toDisk)
    {
        ...
        NSInvocationOperation *operation = SDWIReturnAutoreleased([[NSInvocationOperation alloc] initWithTarget:self  selector:@selector(storeKeyWithDataToDisk:)
            object:keyWithData]);
